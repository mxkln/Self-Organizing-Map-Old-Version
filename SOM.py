# -*- coding: utf-8 -*-
"""
Created on Thu Mar 23 10:48:16 2023
...
def calculate_neighbours_with_gaussian_alpha(b, i, radius, alpha): # calculates all neighbours of ith particle in b within radius and ohmis gaussian alpha function 
    ...
    for j in range(len(b)):
        ...
        if dist <= radius:
            ...
            gauss_alpha = gaussian_alpha(dist,radius,alpha)
            neighbours.append(gauss_alpha)
    return neighbours    

def gaussian_alpha(dist,radius,alpha):
    gaussian_alpha = alpha * math.exp(-((dist-radius)**2)/(2*radius**2))
...
def angular_velocity_single(coords,plot):
    ...
    if plot == "yes":
        ...
        circle = plt.Circle((xc,yc),r,fill = False, color='green')
        ax.add_patch(circle)
